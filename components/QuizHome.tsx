...
import { PlusCircle, Play, Edit2, Trash2, LogOut, User, BookOpen, Trophy, Brain, Settings, Download, Globe, Search, Sparkles, HelpCircle, MessageSquare, ShieldAlert, Users, Crown } from 'lucide-react';
...
interface QuizHomeProps {
  ...
  onOpenFeedback: () => void;
  onViewAdmin?: () => void;
  onHostSession?: (quiz: Quiz) => void; // New
  onViewLeaderboard?: () => void; // New
}
    ...
    <div className="min-h-screen pb-20 bg-[#f1f5f9]">
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6 mb-16 stagger-in">
          <button onClick={onCreateNew} className="lg:col-span-4 bg-gradient-to-br from-purple-500 to-indigo-600 p-8 sm:p-10 rounded-[3.5rem] text-white flex flex-col justify-between group click-scale shadow-2xl min-h-[320px] relative overflow-hidden transition-all duration-500 hover:shadow-purple-200">
            ...
            <div key={quiz.id} className="group bg-white rounded-[3rem] p-8 hover:shadow-2xl transition-all duration-500 flex flex-col relative border border-slate-100 hover-lift">
                <div className={`h-52 rounded-[2.5rem] bg-gradient-to-br from-indigo-500 to-purple-600 mb-10 p-8 flex flex-col justify-between overflow-hidden relative shadow-lg group-hover:shadow-indigo-100 transition-all duration-500`}>
                    ...
                    <div className="flex justify-between items-start z-10">
                        ...
                        {onHostSession && (
                            <button 
                                onClick={(e) => { e.stopPropagation(); onHostSession(quiz); }} 
                                className="bg-indigo-600 text-white p-2.5 rounded-xl shadow-lg hover:bg-indigo-700 transition-all click-scale"
