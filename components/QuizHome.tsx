 ...
 * Level 1: 0-100
 * Level 2: 100-500 (Gap 400)
 * Level 3: 500-1000 (Gap 500)
 * Formula: Each level gap increases by 100.
 */
      ...
      {showLevelModal && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-2xl z-[200] flex items-center justify-center p-6 animate-in fade-in">
              <div className="bg-slate-900 rounded-[3rem] p-10 max-w-md w-full border border-white/10 shadow-2xl relative animate-in zoom-in">
                  <button onClick={() => setShowLevelModal(false)} className="absolute top-6 right-6 p-2 hover:bg-white/5 rounded-full transition-colors text-slate-500">
                      <X size={20} />
                  </button>
                  ...
                  <div className="text-center mb-10">
                      ...
                      <h2 className="text-4xl font-black tracking-tighter mb-2">Level {level}</h2>
                  ...
                  <div className="space-y-8">
                      <div className="bg-white/5 rounded-3xl p-6 border border-white/5">
                          ...
                          <p className="text-center text-xs font-bold text-slate-400">
                              <span className="text-indigo-400">{xpRemaining.toLocaleString()} XP</span> until Level {level + 1}
                          </p>
                      </div>
                  ...
                  <button 
                    ...
                    className="w-full mt-10 py-5 bg-white text-slate-950 rounded-2xl font-black uppercase tracking-widest text-xs click-scale shadow-xl"
                  >
                      Acknowledge
                  </button>
              </div>
      ...
      <header className="glass sticky top-0 z-40 px-6 py-4 flex items-center justify-between border-b border-white/10 animate-in slide-in-from-top duration-500">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="hidden sm:flex items-center gap-3 bg-white/5 border border-white/10 px-3 py-2 rounded-2xl shadow-sm backdrop-blur-sm transition-all hover:bg-white/10 pr-4 click-scale"
          >
            <div className="relative w-10 h-10 flex items-center justify-center">
                <svg className="w-full h-full -rotate-90 transform" viewBox="0 0 36 36">
                    ...
                    <path className="text-indigo-500 transition-all duration-1000 ease-out" strokeDasharray={`${progressPercent}, 100`} d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" strokeWidth="4" strokeLinecap="round" />
                </svg>
          ...
          <button onClick={onOpenFeedback} title="Send Feedback" className="p-3 bg-white/5 border border-white/10 hover:bg-white/10 rounded-2xl transition-all text-slate-400 hover:text-indigo-400 shadow-sm click-scale">
            <MessageSquare size={20} />
          </button>

          <button onClick={onViewSettings} title="Settings" className="p-3 bg-white/5 border border-white/10 hover:bg-white/10 rounded-2xl transition-all text-slate-400 hover:text-purple-400 shadow-sm click-scale">
            <Settings size={20} />
          </button>
          
          <button onClick={onLogout} title="Sign Out" className="p-3 bg-rose-500/10 border border-rose-500/20 hover:bg-rose-500/20 text-rose-500 rounded-2xl transition-all click-scale shadow-sm">
            <LogOut size={20} />
          </button>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
             ...
             <div className="grid grid-rows-2 gap-8">
                <div className="grid grid-cols-2 gap-8">
                   <button onClick={onViewCommunity} className="bg-white/5 p-6 rounded-[2.5rem] border border-white/10 hover:border-purple-500/50 flex flex-col justify-center items-center text-center group click-scale transition-all shadow-sm hover:shadow-lg">
                      ...
                      <h3 className="text-lg font-black tracking-tight text-white leading-tight">Explore</h3>
                   </button>
                ...
                <div className="grid grid-cols-2 gap-8">
                    <button onClick={onViewAchievements} className="bg-white/5 p-6 rounded-[2.5rem] border border-white/10 hover:border-yellow-500/50 flex flex-col justify-center items-center text-center group click-scale transition-all shadow-sm hover:shadow-lg">
                       ...
                       <h3 className="text-lg font-black tracking-tight text-white">Trophies</h3>
                    </button>

                    <button 
                        onClick={handleLuckyDip}
                        disabled={isLoadingRandom}
                        className="bg-emerald-600 p-6 rounded-[2.5rem] border border-emerald-500 flex flex-col justify-center items-center text-center group click-scale transition-all shadow-2xl hover:shadow-emerald-500/20"
                    >
          ...
          <div className="lg:col-span-4 flex flex-col gap-8">
             <button onClick={onJoinGame} className="bg-white p-8 rounded-[3.5rem] flex flex-col justify-between group click-scale transition-all shadow-xl min-h-[160px]">
                <div className="flex items-center gap-4">
                   <div className="w-14 h-14 bg-indigo-600 rounded-2xl flex items-center justify-center text-white group-hover:bg-slate-900 transition-all">
                      <Zap size={28} />
                   </div>
                   <div className="text-left">
                      <h3 className="text-2xl font-black tracking-tight text-slate-950 leading-tight">Join Game</h3>
             ...
             <div className="flex-1 bg-white/5 border border-white/10 rounded-[3.5rem] p-10 flex flex-col justify-between relative overflow-hidden group min-h-[220px]">
                ...
                <div className="relative z-10">
                   <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] mb-4">Recent Activity</h4>
                   {latestResult ? (
                ...
                <button onClick={onViewHistory} className="relative z-10 w-full mt-6 py-4 bg-white hover:bg-slate-100 text-slate-950 rounded-2xl font-black uppercase text-[10px] tracking-widest transition-all click-scale">Open Activity Log</button>
             </div>
          </div>
        </section>
        ...
        <section className="flex flex-col md:flex-row items-center justify-between gap-8 mb-12 stagger-in">
           <div className="bg-white/5 p-1.5 rounded-[2.5rem] flex gap-1 border border-white/10 shadow-inner w-full md:w-auto">
              <button onClick={() => setActiveTab('my')} className={`flex-1 md:flex-none px-10 py-4 rounded-[2rem] font-black text-sm transition-all ${activeTab === 'my' ? 'bg-indigo-600 text-white shadow-xl scale-105' : 'text-slate-500 hover:text-slate-300'}`}>My Quizzes</button>
              <button onClick={() => setActiveTab('saved')} className={`flex-1 md:flex-none px-10 py-4 rounded-[2rem] font-black text-sm transition-all ${activeTab === 'saved' ? 'bg-indigo-600 text-white shadow-xl scale-105' : 'text-slate-500 hover:text-slate-300'}`}>Saved Quizzes</button>
           </div>
           ...
           <div className="relative w-full md:w-96 group">
             ...
             <input type="text" placeholder="Search library..." value={searchQuery} onChange={(e) => setSearchQuery((e.target as any).value)} className="w-full pl-14 pr-8 py-5 rounded-[2.5rem] border border-white/10 bg-white/5 focus:bg-white/10 focus:outline-none focus:border-purple-500 transition-all font-bold text-white shadow-sm placeholder:text-slate-600" />
           </div>
        </section>
            ...
            <TiltCard key={quiz.id} className="group bg-white/5 rounded-[3rem] p-8 transition-all duration-500 flex flex-col relative border border-white/10 h-full hover:border-indigo-500/30">
                <div className={`h-52 rounded-[2.5rem] bg-gradient-to-br from-indigo-600 to-purple-800 mb-10 p-8 flex flex-col justify-between overflow-hidden relative shadow-lg group-hover:shadow-indigo-500/10 transition-all duration-500 transform-style-3d border border-white/10`}>
                    ...
                    <div className="flex justify-between items-start z-10">
                        <div className="bg-black/40 backdrop-blur-md text-white text-[10px] font-black uppercase tracking-widest px-4 py-1.5 rounded-full border border-white/10">
                ...
                <div className="mt-auto grid grid-cols-2 gap-4">
                  <button onClick={() => setPendingQuizPlay(quiz)} className="bg-white text-slate-950 font-black py-5 rounded-[1.5rem] flex items-center justify-center gap-2 click-scale shadow-lg hover:bg-slate-100 transition-all">
                    <Play size={18} fill="currentColor" /> Play
                  </button>
                  <button onClick={() => onStartStudy(quiz)} className="bg-white/5 border border-white/10 text-slate-300 font-black py-5 rounded-[1.5rem] flex items-center justify-center gap-2 click-scale hover:bg-white/10 transition-all">
        ...
        {hasMore && (
            <div className="flex justify-center mt-12 mb-8">
                <button 
                    ...
                    className="px-12 py-4 bg-white/5 border border-white/10 hover:border-indigo-500/50 text-indigo-400 font-black rounded-2xl shadow-sm hover:shadow-lg transition-all click-scale flex items-center gap-2 uppercase tracking-widest text-xs"
                >
                    <ChevronDown size={18} />
                    Load More Modules
                </button>
