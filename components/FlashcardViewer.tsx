...
import { ArrowLeft, RotateCw, X, ThumbsUp, ThumbsDown, Repeat, CheckCircle, Brain, Shuffle } from 'lucide-react';
...
export const FlashcardViewer: React.FC<FlashcardViewerProps> = ({ quiz, onExit }) => {
  ...
  const startSession = (questionsToStudy: Question[]) => {
    // Shuffle questions for better study effect
  ...
  // Keyboard Shortcuts
  useEffect(() => {
    // Fix: Cast KeyboardEvent to any
    const handleKeyDown = (e: any) => {
      if (studyState !== 'studying') return;

      if (e.code === 'Space') {
        // Fix: Use e.preventDefault
  ...
  }, [isFlipped, studyState, currentCardIndex]); // Dependencies needed for closure
  ...
  const handleGrade = (known: boolean) => {
    ...
    if (currentCardIndex < studyQueue.length - 1) {
      ...
      (window as any).setTimeout(() => setCurrentCardIndex(prev => prev + 1), 150); // Small delay for animation feel
    } else {
      setStudyState('summary');
    }
  };

  // Render content on the back of the card based on question type
        ...
        return (
            <div className="w-full space-y-2">
                ...
                {q.options.map((opt, idx) => {
                    ...
                    return (
                        <div key={idx} className={`p-3 rounded-xl flex items-center justify-between border-2 ${
                            isCorrect 
    ...
    if (q.type === 'slider') {
        return (
            <div className="text-center">
                <p className="text-sm font-bold opacity-50 uppercase mb-4">Exact Value</p>
  ...
  if (studyState === 'summary') {
    const accuracy = Math.round((knownCount / studyQueue.length) * 100);
    
    return (
        <div 
            className={`min-h-screen flex items-center justify-center p-4 ${customClass}`}
            style={customStyle}
        >
            <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full text-center animate-in zoom-in duration-300">
                ...
                <div className="space-y-3">
                    {unknownQuestions.length > 0 && (
                        <button 
                            onClick={() => startSession(unknownQuestions)}
                            className="w-full bg-orange-500 hover:bg-orange-600 text-white font-bold py-4 rounded-xl shadow-lg hover:shadow-orange-200 transition-all flex items-center justify-center gap-2"
                        >
                            <Repeat size={20} />
                            Study Missed Cards ({unknownQuestions.length})
                        </button>
                    )}
                    
                    <button 
                         onClick={() => startSession(quiz.questions)}
                    ...
                    <button 
                        onClick={onExit}
                        className="w-full bg-white border-2 border-slate-200 hover:bg-slate-50 text-slate-600 font-bold py-4 rounded-xl transition-all"
                    >
                        Back to Dashboard
                    </button>
                </div>
  ...
  return (
    <div 
        className={`min-h-screen flex flex-col overflow-hidden ${customClass}`}
        style={customStyle}
    >
      {/* Header */}
      <div className="bg-white/10 backdrop-blur-md px-4 py-3 flex items-center justify-between shadow-lg border-b border-white/10 z-10">
        ...
        <button
          onClick={onExit}
          className="bg-white bg-opacity-10 hover:bg-opacity-20 p-2 rounded-lg transition-colors"
        >
          <X size={24} />
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4 sm:p-6 perspective-1000">
         ...
         <div 
            className="relative w-full max-w-xl aspect-[3/4] sm:aspect-[4/3] cursor-pointer group perspective-1000 transition-all duration-500"
         ...
         >
            ...
            <div className={`relative w-full h-full duration-500 preserve-3d transition-transform ${isFlipped ? 'rotate-y-180' : ''}`}>
                ...
                {/* --- FRONT --- */}
                <div className="absolute w-full h-full backface-hidden bg-white text-slate-900 rounded-3xl shadow-2xl p-6 sm:p-10 flex flex-col items-center justify-between border-b-8 border-slate-200">
                    <div className="w-full flex justify-between items-start">
                        ...
                        <span className="text-xs font-bold text-slate-400 flex items-center gap-1 animate-pulse">
                            <RotateCw size={12} /> Tap to Flip / Space
                        </span>
                    </div>
                    
                    <div className="flex-1 flex flex-col items-center justify-center w-full text-center">
                        <h3 className={`${currentQ.question.length > 50 ? 'text-xl sm:text-2xl' : 'text-2xl sm:text-3xl'} font-black text-slate-800 mb-6`}>
                            {currentQ.question}
                        </h3>
                        {currentQ.image && (
                            <img 
                                src={currentQ.image} 
                                alt="" 
                                className="max-h-48 rounded-lg object-contain shadow-sm border border-slate-100" 
                            />
                        )}
                    </div>
                ...
                {/* --- BACK --- */}
                <div className="absolute w-full h-full backface-hidden rotate-y-180 bg-white text-slate-900 rounded-3xl shadow-2xl p-6 sm:p-10 flex flex-col items-center border-b-8 border-indigo-200 overflow-y-auto custom-scrollbar">
         ...
         <div className="w-full max-w-xl mt-8 h-20">
            {isFlipped ? (
                <div className="grid grid-cols-2 gap-4 h-full animate-in slide-in-from-bottom-4 fade-in duration-300">
                    <button 
                        onClick={() => handleGrade(false)}
                        className="bg-orange-100 hover:bg-orange-200 border-2 border-orange-200 text-orange-700 rounded-2xl font-bold text-lg flex flex-col items-center justify-center transition-all active:scale-95"
                    >
                    ...
                    <button 
                        ...
                        className="bg-green-100 hover:bg-green-200 border-2 border-green-200 text-green-700 rounded-2xl font-bold text-lg flex flex-col items-center justify-center transition-all active:scale-95"
                    >
                ...
                <button 
                    ...
                    className="w-full h-full bg-white/20 hover:bg-white/30 text-white border-2 border-white/30 rounded-2xl font-bold text-xl backdrop-blur-sm transition-all shadow-lg active:scale-95"
                >
                    Show Answer
                </button>
            )}
