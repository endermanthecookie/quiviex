...
import { ArrowLeft, Calendar, History, Trophy, Clock, X, Info, Download } from 'lucide-react';
...
export const HistoryPage: React.FC<HistoryPageProps> = ({ user, onBack }) => {
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Date', 'Time', 'Quiz Title', 'Score', 'Total Questions', 'Accuracy', 'Points'];
    const rows = history.map(h => {
        const d = new Date(h.date);
        return [
            d.toLocaleDateString(),
            d.toLocaleTimeString(),
            `"${h.quizTitle.replace(/"/g, '""')}"`, // Escape quotes
            h.score,
            h.totalQuestions,
    ...
    link.href = url;
    link.setAttribute('download', `quiviex_history_${user.username}_${new Date().toISOString().split('T')[0]}.csv`);
        ...
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          ...
          {history.length > 0 && (
              <button 
                ...
                className="flex items-center gap-2 bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-xl font-bold text-xs uppercase tracking-widest transition-all click-scale"
                title="Download CSV"
              >
