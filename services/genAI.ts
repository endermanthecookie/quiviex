 ...
 * Purged all Gemini-related SDK references.
 */
...
const getEndpoint = (prefs: UserPreferences | undefined, mode: 'text' | 'image') => {
    ...
    if (mode === 'text') {
        ...
        // GitHub doesn't have an image gen REST endpoint like OpenAI yet (mostly chat)
        // so we assume OpenAI/DALL-E for images unless explicitly changed.
...
export const generateImageForQuestion = async (text: string, prefs: UserPreferences | undefined): Promise<string | null> => {
    ...
    if (provider === 'github') return null; // GitHub Models doesn't support direct image generation endpoints yet
